...
export default function UploadCategoryDocuments({ id,userCategory }) {
  ...
  const { uploadChecklistDocument} = useClientManagementApi();
  const {uploadCategoryDocument,loading}=useTaxfilerApi()
  const now = new Date();
  ...
  const handleUpload = async () => {
    ...
    try {
      for (const fileItem of files) {
        await uploadCategoryDocument(id, fileItem.file, title, selectedYear, selectedMonth,userCategory);
      }
