...
export default function AllPaymentsListing() {
  ...
  const { getAllPayments, loading } = usePaymentApi();
  ...
  const fetchPayments = async () => {
    try {
      const response = await getAllPayments(currentPage, rowsPerPage, search);
      if (response && response.success) {
        setPayments(response.payload?.data || []);
        setTotalRows(response.payload?.meta?.total_results || (response.payload?.data || []).length);
      }
    } catch (err) {
      console.error(err);
    }
    ...
    <PageContainer className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Showing all listing for payments</p>
        </div>
        <Button
          onClick={() => router.push(ROUTES.individual.paymentsAdd)}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Select value={taxYearFilter} onValueChange={(v) => { setTaxYearFilter(v); setCurrentPage(1); }}>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="p-3">
            ...
            />
            {dateFilter && (
              <DropdownMenuItem onClick={() => { setDateFilter(""); setCurrentPage(1); }} className="mt-2 justify-center text-muted-foreground">
                Clear date
              </DropdownMenuItem>
            )}
          </DropdownMenuContent>
          ...
          <TableBody>
            {loading ? (
              ...
              filteredPayments.map((payment, index) => {
                const clientName = payment.client ? `${payment.client.first_name || ""} ${payment.client.last_name || ""}`.trim() : "Unknown Client";
                const pDate = payment.payment_date?.includes("T") ? payment.payment_date.split("T")[0] : payment.payment_date;
                  ...
                  <TableRow key={payment.id}>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <div
                          className={`h-9 w-9 rounded-full flex items-center justify-center text-sm font-medium ${getAvatarStyle(index)}`}
                        >
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem
                            onClick={() => router.push(ROUTES.individual.paymentsView + `?taxfilerId=${payment.client_id}&paymentId=${payment.id}`)}
                          >
                            <Eye className="mr-2 h-4 w-4" />
                            View
                          </DropdownMenuItem>
                          <DropdownMenuItem
                            onClick={() => router.push(ROUTES.individual.paymentsEdit + `?taxfilerId=${payment.client_id}&paymentId=${payment.id}`)}
                          >
