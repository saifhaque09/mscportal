...
export default function ClientAccountantEnrolment({ firmGuid, onBack }) {
  const {
    getFirmMembers,
    firmMembers,
    firmMembersMeta,
  ...
  }, [getFirmMembers, firmGuid, currentPage, rowsPerPage, debouncedSearch]);
  ...
  useEffect(() => {
    if (firmGuid) fetchMembers();
  }, [fetchMembers, firmGuid]);

  const members = normalizeMembers(firmMembers, roleLabels);
  const totalPages = firmMembersMeta?.last_page ?? 1;
  const totalResults = firmMembersMeta?.total_results ?? 0;

  // True when a Lead Accountant is already assigned to this firm
  const hasLeadAccountant = members.some((m) => m.role === "Lead Accountant");

  // IDs of users already assigned any role in this firm
  const assignedUserIds = new Set(members.map((m) => m.id));

  // Accountants not yet assigned any role in this firm — the only ones selectable in the modal
  const availableAccountantUsers = accountantUser.filter((u) => !assignedUserIds.has(u.id));
    ...
    <Card className="border-border shadow-sm mb-6">
      <CardContent className="p-6">
        <div className="space-y-3">
          <div className="flex">
            <span className="font-medium w-48">Business Name:</span>
            <span className="text-muted-foreground">
              {firmMembers?.firm_name ?? "—"}
            </span>
          </div>
          <div className="flex">
            <span className="font-medium w-48">Business GUID:</span>
            <span className="text-muted-foreground">
              {firmMembers?.guid ?? firmGuid ?? "—"}
            </span>
          </div>
        </div>
      </CardContent>
          ...
          <div className="space-y-3 py-2">
            ...
            <div className="border border-border rounded-md overflow-hidden">
              <div className="max-h-72 overflow-y-auto">
                ...
                ) : availableAccountantUsers.length === 0 ? (
                  <div className="py-8 text-center text-sm text-muted-foreground">
                    No accountants found
                  </div>
                ) : (
                  availableAccountantUsers.map((u) => {
                    const fullName = `${u.first_name ?? ""} ${u.last_name ?? ""}`.trim();
                    return (
                      <div
                        key={u.id}
                        onClick={() => handleSelectFromModal(u)}
                        className="flex items-center gap-3 px-4 py-3 border-b border-border last:border-b-0 transition-colors cursor-pointer hover:bg-muted/50"
                      >
                        <div
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="flex justify-end gap-2 pb-2 px-1">
            ...
            <Button
              onClick={async () => {
                const result = await updateFirmMemberRole({
                  guid: firmGuid,
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="py-4 space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              This will remove the member from this organisation. This action cannot be undone.
            </p>
          </div>
          <div className="flex justify-end gap-2 pb-2 px-1">
            <Button
            ...
            <Button
              ...
              onClick={async () => {
                ...
                const result = await removeFirmMember({
                  guid: firmGuid,
